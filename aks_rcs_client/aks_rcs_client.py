...
class RCSclient(Node):
    def __init__(self):
        ...
        self.declare_parameters(
            ...
            parameters=[
                ...
                ('team_id', "team_id"),                         # Used for client name and mqtt kart state topic
                ('mqtt_track_state_topic', "track_state"),
                ('mqtt_kart_state_topic', "kart_state"),
                ('ros_track_state_topic', "track_state"),       # Topic to publish track state within ROS
                ('ros_kart_state_topic', "kart_state")          # Topic to get kart state within ROS
            ]
        )
	...
	# Set Parameter values to variables
        ...
        self.mqtt_kart_topic = str(self.get_parameter('mqtt_kart_state_topic').value) + "/" + str(self.get_parameter('team_id').value) 
	...
	# Initalize ros2 publisher and subscriber
        ...
        self.race_kart_sub = self.create_subscription(String, ros_kart_topic, self.listener_callback, 10) 
